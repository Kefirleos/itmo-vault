...
3. [Внутренняя архитектура Git (Under the Hood)](#3-внутренняя-архитектура-git-under-the-hood)
   ...
   - [Четыре фундаментальных объекта Git (Blob, Tree, Commit, Tag)](#33-четыре-фундаментальных-объекта-git-blob-tree-commit-tag)
...
4. [Консольный рабочий процесс и основные команды](#4-консольный-рабочий-процесс-и-основные-команды)
   - [Инициализация и клонирование (init, clone)](#41-инициализация-и-клонирование-init-clone)
   - [Фиксация изменений (add, commit, commit --amend)](#42-фиксация-изменений-add-commit-commit---amend)
   - [Навигация и управление ветками (status, branch, checkout, merge)](#43-навигация-и-управление-ветками-status-branch-checkout-merge)
   - [Удаленные репозитории (pull, push, log)](#44-удаленные-репозитории-pull-push-log)
...
5. [Модель ветвления GitFlow](#5-модель-ветвления-gitflow)
   ...
   - [Жизненный цикл разработки по GitFlow](#52-жизненный-цикл-разработки-по-gitflow)
...
Большинство систем (CVS, Subversion) хранят информацию в виде списка файлов и набора изменений (дельт) для каждого файла во времени:

```text
CVS / SVN (Delta-based):
Файл A:  [Версия 1] ---> [Δ1] --------> [Δ2]
Файл B:  [Версия 1] ------------------> [Δ1]
Файл C:  [Версия 1] ---> [Δ1] ---> [Δ2]
```

Git рассматривает данные как **набор снимков состояния (snapshots)** всей файловой системы проекта:

```text
Git (Snapshot-based):
Коммит 1:  [A1] -------> [B1] -------> [C1]
            │             │             │
Коммит 2:  [A2] ──┐      [B1]*         [C2]   (*ссылка на неизменный B1)
            │     │       │             │
Коммит 3:  [A2]*  └───>  [B2] ───────> [C3]
```

Если файл не был изменен в текущей версии, Git не дублирует его, а просто сохраняет жесткую ссылку на уже существующий неизменный объект.
